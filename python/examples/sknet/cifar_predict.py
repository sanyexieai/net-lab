    ...
    for root, _, files in os.walk(directory):
        for file in files:
            if file.lower().endswith(('.png', '.jpg', '.jpeg')):
                ...
                try:
                    predicted_class, probability = predict_image(model_path, image_path, device)
                    print(f"\n图片: {rel_path}")
...
def main():
    ...
    model_path = os.path.join(model_dir, 'sknet_cifar.safetensors')
    ...
    # 获取测试图片路径
    image_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../data/test_images/cifar10'))
